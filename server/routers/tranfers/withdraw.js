...
const fs = require('fs')
// var withdraw = require("./withdrawmodel");
const apiKey = 'ef5955d807a91afa822fe49542d41c2e'
// GET all
// router.get("/", (req, res) => {
//     withdraw.find().exec((err, data) => {
...
//     withdraw.findById(req.params._id).exec((err, data) => {
//     if (err) return res.status(400).send(err);
//     res.status(200).send(data);
//   });
// });
...
router.put("/:_id", (req, res) => {
    withdraw.findByIdAndUpdate(req.params._id, req.body, (err, data) => {
    if (err) return res.status(400).send(err);
...
router.delete("/:_id", (req, res) => {
    withdraw.findByIdAndDelete(req.params._id, (err, data) => {
    if (err) return res.status(400).send(err);
